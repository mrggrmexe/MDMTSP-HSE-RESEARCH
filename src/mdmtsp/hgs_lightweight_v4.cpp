...
constexpr std::size_t kHgsV4RelinkSampleCap = 24;
constexpr std::size_t kHgsV4RelinkBaseBudget = 2;
constexpr cost_t kHgsV4RelinkRatio = static_cast<cost_t>(0.00030);
constexpr std::size_t kHgsV4RelinkMaxBudget = 10;
constexpr std::size_t kHgsV4StagnationThreshold = 40;
constexpr std::size_t kHgsV4HighStagnationThreshold = 96;
...
[[nodiscard]] std::vector<depot_id_t> hgs_mutate_assignment(
    ...
    const std::vector<std::vector<std::size_t>>& orders,
    HgsAnnealRng& arng,
    const std::size_t extra_moves = 0
) {
    auto out = assignment;
    const std::size_t base_limit = instance.customer_count() <= 100 ? 2U : 3U;
    const std::size_t moves = 1U + arng.next_index(base_limit) + extra_moves;
...
[[nodiscard]] std::vector<depot_id_t> hgs_path_relink_towards_elite(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    const std::vector<std::array<depot_id_t, 2>>& nearest_two_depots,
    ...
    if (elite_assignment == nullptr || assignment.empty() || elite_assignment->size() != assignment.size()) {
        return assignment;
    }

    auto out = assignment;
    auto depot_loads = hgs_assignment_depot_loads(instance, out);
    std::vector<std::size_t> diff_positions;
    diff_positions.reserve(out.size());
    for (std::size_t i = 0; i < out.size(); ++i) {
        if (out[i] != (*elite_assignment)[i]) {
    ...
    const std::size_t budget = std::min<std::size_t>(
        std::max<std::size_t>(
            kHgsV4RelinkBaseBudget,
            static_cast<std::size_t>(std::ceil(kHgsV4RelinkRatio * static_cast<cost_t>(instance.customer_count())))
        ),
        std::min<std::size_t>(kHgsV4RelinkMaxBudget, diff_positions.size())
    );

    for (std::size_t step = 0; step < budget && !diff_positions.empty(); ++step) {
        const std::size_t sample_count = std::min<std::size_t>(kHgsV4RelinkSampleCap, diff_positions.size());
        std::size_t best_pick = diff_positions.size();
        cost_t best_gain = 0.0;

        for (std::size_t s = 0; s < sample_count; ++s) {
            const std::size_t pick = arng.next_index(diff_positions.size());
            const std::size_t customer_idx = diff_positions[pick];
            const depot_id_t current_depot = out[customer_idx];
            const depot_id_t target_depot = (*elite_assignment)[customer_idx];
            if (current_depot == target_depot || current_depot >= instance.depot_count() || target_depot >= instance.depot_count()) {
                continue;
            }
            if (depot_loads[current_depot] == 0U) {
                continue;
            ...
            const cost_t target_score = hgs_assignment_local_depot_score(
                instance,
                matrix,
                salesmen_per_depot,
                depot_loads,
            ...
            const cost_t gain = current_score - target_score;
        ...
        if (best_pick >= diff_positions.size()) {
            break;
        }

        const std::size_t customer_idx = diff_positions[best_pick];
        const depot_id_t current_depot = out[customer_idx];
        const depot_id_t target_depot = (*elite_assignment)[customer_idx];
        if (current_depot < instance.depot_count() && depot_loads[current_depot] > 0U) {
            --depot_loads[current_depot];
        }
        out[customer_idx] = target_depot;
        ++depot_loads[target_depot];
        diff_positions[best_pick] = diff_positions.back();
        diff_positions.pop_back();
    }
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_hgs_lightweight_v4(
    ...
    if (population.empty()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v4: empty initial population");
    }
    ...
    while (child_count < child_limit && std::chrono::steady_clock::now() < deadline) {
        ...
        const std::size_t extra_mutation_moves =
            stagnation_children >= kHgsV4HighStagnationThreshold ? 2U :
            (stagnation_children >= kHgsV4StagnationThreshold ? 1U : 0U);
        ...
        child_assignment = hgs_path_relink_towards_elite(
            instance,
            matrix,
            salesmen_per_depot,
            nearest_two_depots,
        ...
        const bool proxy_improved = child.proxy < best_proxy_seen - kEps;
        if (proxy_improved) {
            best_proxy_seen = child.proxy;
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v4: failed to build any candidate");
    }
