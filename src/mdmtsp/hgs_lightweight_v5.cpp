...
constexpr cost_t kHgsV5MateDiversityHammingWeight = static_cast<cost_t>(0.020);
constexpr cost_t kHgsV5MateDiversityLoadWeight = static_cast<cost_t>(0.015);
constexpr std::size_t kHgsV5MateSampleCount = 4;
constexpr std::size_t kHgsV5StagnationThreshold = 48;
constexpr std::size_t kHgsV5HighStagnationThreshold = 96;
constexpr cost_t kHgsV5NearCloneProxySlack = static_cast<cost_t>(0.0025);
...
[[nodiscard]] std::size_t select_diverse_mate(
    ...
    const std::size_t rounds = std::min<std::size_t>(kHgsV5MateSampleCount, population.size() - 1U);
    ...
    for (std::size_t r = 0; r < rounds * 2U; ++r) {
        ...
        const std::size_t hamming = assignment_hamming_distance(
            population[anchor_idx].assignment,
            population[cand].assignment
        );
        const std::size_t load_dist = hgs_load_l1_distance(
            population[anchor_idx].depot_loads,
            population[cand].depot_loads
        );
        const cost_t rank =
            score(population[cand]) -
            kHgsV5MateDiversityHammingWeight * static_cast<cost_t>(hamming) -
            kHgsV5MateDiversityLoadWeight * static_cast<cost_t>(load_dist);
...
[[nodiscard]] std::size_t hgs_near_clone_hamming_threshold(const std::size_t customer_count) noexcept {
    if (customer_count <= 50) {
        return 2;
    }
    if (customer_count <= 250) {
...
[[nodiscard]] std::size_t hgs_near_clone_load_threshold(const std::size_t depot_count) noexcept {
    return depot_count <= 2 ? 1U : 2U;
}

[[nodiscard]] bool hgs_is_dominated_near_clone(
    const std::vector<HgsIndividual>& population,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& depot_loads,
    const cost_t proxy
) noexcept {
    const std::size_t hamming_threshold = hgs_near_clone_hamming_threshold(assignment.size());
    const std::size_t load_threshold = hgs_near_clone_load_threshold(depot_loads.size());

    for (const auto& ind : population) {
        const cost_t incumbent_score = ind.materialized ? ind.value : ind.proxy;
        if (incumbent_score > proxy * (static_cast<cost_t>(1.0) + kHgsV5NearCloneProxySlack) + kEps) {
            continue;
        }
        const std::size_t hamming = assignment_hamming_distance(assignment, ind.assignment);
        if (hamming > hamming_threshold) {
            continue;
        }
        const std::size_t load_dist = hgs_load_l1_distance(depot_loads, ind.depot_loads);
        if (load_dist <= load_threshold) {
            return true;
        }
    }
    return false;
...
[[nodiscard]] std::vector<depot_id_t> hgs_assignment_crossover(
    ...
    const std::size_t patch_count = std::min<std::size_t>(std::max<std::size_t>(1, instance.customer_count() / 20), 8);
    for (std::size_t p = 0; p < patch_count; ++p) {
        const std::size_t idx = arng.next_index(order.size());
        child[order[idx]] = parent_a[order[idx]];
    }

    const std::size_t boundary_budget = std::min<std::size_t>(
        ...
        std::min<std::size_t>(24U, instance.customer_count())
    );

    std::size_t applied = 0;
    for (std::size_t t = 0; t < order.size() && applied < boundary_budget; ++t) {
        const std::size_t customer_idx = order[t];
        if (parent_a[customer_idx] == parent_b[customer_idx]) {
            continue;
        }
        const node_id_t customer = instance.customer_node_from_index(customer_idx);
        const depot_id_t depot_a = parent_a[customer_idx];
        const depot_id_t depot_b = parent_b[customer_idx];
        child[customer_idx] =
            (matrix[depot_a][customer] <= matrix[depot_b][customer]) ? depot_a : depot_b;
        ++applied;
    }
...
[[nodiscard]] std::vector<depot_id_t> hgs_mutate_assignment(
    ...
    const std::vector<std::vector<std::size_t>>& orders,
    HgsAnnealRng& arng,
    const std::size_t extra_moves = 0U
) {
    auto out = assignment;
    const std::size_t base_moves = 1U + arng.next_index(instance.customer_count() <= 100 ? 2U : 3U);
    const std::size_t moves = base_moves + extra_moves;
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_hgs_lightweight_v5(
    ...
    if (population.empty()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v5: empty initial population");
    }
    ...
    std::size_t stagnation_children = 0;
    while (child_count < child_limit && std::chrono::steady_clock::now() < deadline) {
        ++child_count;
        const std::size_t ia = tournament_pick(population, arng);
        std::size_t ib = select_diverse_mate(population, ia, arng);
        if (population.size() > 1) {
            while (ib == ia) {
                ib = tournament_pick(population, arng);
            }
        }
        ...
        const std::size_t extra_mutation_moves =
            stagnation_children >= kHgsV5HighStagnationThreshold ? 2U :
            (stagnation_children >= kHgsV5StagnationThreshold ? 1U : 0U);
        child_assignment = hgs_mutate_assignment(
            instance,
            matrix,
            child_assignment,
            orders,
            arng,
            extra_mutation_moves
        );
        ...
        if (hgs_is_dominated_near_clone(population, child.assignment, child.depot_loads, child.proxy)) {
            ++stagnation_children;
            continue;
        }

        const cost_t previous_best_proxy = best_proxy_seen;
        ...
        if (best_proxy_seen < previous_best_proxy - kEps || improved_solution) {
            stagnation_children = 0;
        } else {
            ++stagnation_children;
        }
    }
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v5: failed to build any candidate");
    }
