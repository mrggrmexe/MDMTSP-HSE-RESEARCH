...
constexpr std::size_t kSaV4SmallDelegationThreshold = 160;
constexpr std::size_t kSaV4PortfolioFinalizeThreshold = 720;
constexpr std::size_t kSaV4RepairCustomerThreshold = 600;
constexpr std::size_t kSaV4MediumBaselineThreshold = 420;
...
[[nodiscard]] std::vector<depot_id_t> assignment_from_solution(
    const MDMTSPInstance& instance,
    const MDMTSPSolution& solution
) {
    std::vector<depot_id_t> assignment(instance.customer_count(), depot_id_t{0});

    for (const auto& salesman_route : solution.routes) {
        const depot_id_t depot = salesman_route.depot_id;
        for (const node_id_t node : salesman_route.nodes) {
            if (!instance.is_customer_node(node)) {
                continue;
            }
            const std::size_t customer_idx = instance.customer_index_from_node(node);
            if (customer_idx < assignment.size()) {
                assignment[customer_idx] = depot;
            }
        }
    }
...
[[nodiscard]] std::uint64_t make_anneal_seed(
    const MDMTSPInstance& instance,
    const std::vector<depot_id_t>& assignment
) noexcept {
...
[[nodiscard]] std::int64_t sa_time_budget_ms(const std::size_t customer_count) noexcept {
...
[[nodiscard]] std::size_t sa_block_limit(const std::size_t customer_count) noexcept {
    if (customer_count <= 50) {
        return 2;
    }
    if (customer_count <= 500) {
...
[[nodiscard]] std::size_t sa_materialize_stride(const std::size_t customer_count) noexcept {
    if (customer_count <= 50) {
        return 2;
    }
...
[[nodiscard]] std::size_t sa_max_attempts(const std::size_t customer_count) noexcept {
    if (customer_count <= 50) {
        return 1500;
    }
    if (customer_count <= 150) {
...
[[nodiscard]] std::size_t sa_repair_stagnation_limit(const std::size_t customer_count) noexcept {
    if (customer_count <= 150) {
        return 24;
    }
    if (customer_count <= 1000) {
...
[[nodiscard]] std::size_t sa_nearest_depot_candidate_count(const MDMTSPInstance& instance) noexcept {
    return std::min<std::size_t>(instance.depot_count(), 3U);
}

[[nodiscard]] cost_t assignment_access_proxy(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& salesmen_per_depot
    ...
    cost_t total = 0.0;
    for (std::size_t customer_idx = 0; customer_idx < instance.customer_count(); ++customer_idx) {
        const depot_id_t depot = assignment[customer_idx];
        const node_id_t customer = instance.customer_node_from_index(customer_idx);
        const cost_t d = matrix[depot][customer];
        total += d;
        ++depot_loads[depot];
        depot_sum_dist[depot] += d;
    }
    ...
    for (depot_id_t depot = 0; depot < instance.depot_count(); ++depot) {
        ...
        total += static_cast<cost_t>(1.2) * mean_dist;
        ...
        total += static_cast<cost_t>(0.03) * overload * overload;
    }

    return total;
}

[[nodiscard]] cost_t anneal_solution_value(const MDMTSPSolution& solution) noexcept {
    return solution.feasible ? solution.objective : solution.objective + static_cast<cost_t>(1e12);
}

[[nodiscard]] std::vector<std::vector<std::size_t>> sa_customer_orders(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix
) {
...
[[nodiscard]] std::vector<cost_t> ideal_depot_loads(
    const MDMTSPInstance& instance,
    const std::vector<std::size_t>& salesmen_per_depot
) {
    std::vector<cost_t> ideal(instance.depot_count(), 0.0);
    const std::size_t total_salesmen = std::max<std::size_t>(1U, instance.salesman_count);
    for (depot_id_t depot = 0; depot < instance.depot_count(); ++depot) {
        ideal[depot] =
            static_cast<cost_t>(instance.customer_count()) *
...
[[nodiscard]] std::vector<std::size_t> assignment_depot_loads(
    const MDMTSPInstance& instance,
    const std::vector<depot_id_t>& assignment
) {
...
void apply_assignment_changes_to_loads(
    std::vector<std::size_t>& loads,
    const std::vector<AssignmentChange>& changes
) {
    for (const auto& change : changes) {
        if (change.from == change.to) {
            continue;
        }
        if (change.from < loads.size() && loads[change.from] > 0) {
            --loads[change.from];
        }
        if (change.to < loads.size()) {
            ++loads[change.to];
        }
    }
}

[[nodiscard]] std::vector<std::vector<depot_id_t>> build_sa_nearest_depot_candidates(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::size_t keep
) {
    std::vector<std::vector<depot_id_t>> nearest(instance.customer_count());
    const std::size_t limit = std::min<std::size_t>(keep, instance.depot_count());
    ...
    for (std::size_t customer_idx = 0; customer_idx < instance.customer_count(); ++customer_idx) {
        ...
        ranked.reserve(instance.depot_count());
        for (depot_id_t depot = 0; depot < instance.depot_count(); ++depot) {
            ranked.emplace_back(matrix[depot][customer], depot);
        }
        std::sort(ranked.begin(), ranked.end(), [](const auto& lhs, const auto& rhs) {
            if (lhs.first != rhs.first) {
                return lhs.first < rhs.first;
        ...
        nearest[customer_idx].reserve(limit);
        for (std::size_t i = 0; i < limit; ++i) {
            nearest[customer_idx].push_back(ranked[i].second);
        }
    }
...
[[nodiscard]] std::array<depot_id_t, 2> nearest_two_depots_for_customer(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    ...
    depot_id_t best = 0;
    depot_id_t second = 0;
    cost_t best_dist = std::numeric_limits<cost_t>::infinity();
    cost_t second_dist = std::numeric_limits<cost_t>::infinity();
...
[[nodiscard]] std::optional<std::size_t> pick_customer_assigned_to_depot(
    const std::vector<depot_id_t>& assignment,
    const depot_id_t depot,
    AnnealRng& arng
) {
    ...
    const std::size_t attempts = std::min<std::size_t>(assignment.size(), 16U);
    for (std::size_t trial = 0; trial < attempts; ++trial) {
        const std::size_t idx = arng.next_index(assignment.size());
        if (assignment[idx] == depot) {
            return idx;
        }
    }

    std::size_t seen = 0;
    const std::size_t offset = arng.next_index(assignment.size());
    for (std::size_t step = 0; step < assignment.size(); ++step) {
        const std::size_t idx = (offset + step) % assignment.size();
        if (assignment[idx] != depot) {
            continue;
        }
        if (seen == 0 || arng.next_index(seen + 1U) == 0U) {
            seen = seen + 1U;
            return idx;
        }
        ++seen;
...
[[nodiscard]] depot_id_t select_source_depot_for_guided_move(
    const std::vector<std::size_t>& loads,
    const std::vector<cost_t>& ideal,
    AnnealRng& arng
) {
    depot_id_t best = 0;
    cost_t best_overload = -std::numeric_limits<cost_t>::infinity();
    for (depot_id_t depot = 0; depot < loads.size(); ++depot) {
        const cost_t overload = static_cast<cost_t>(loads[depot]) - ideal[depot];
        if (overload > best_overload + kEps) {
            best_overload = overload;
            best = depot;
        }
    }

    if (best_overload > kEps && (arng.next_u64() & 3ULL) != 0ULL) {
        return best;
    }

    return static_cast<depot_id_t>(arng.next_index(loads.size()));
}

[[nodiscard]] AssignmentProposal make_block_follow_proposal(
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& order,
    const std::size_t start,
    const std::size_t block_size,
    const bool follow_previous
) {
    AssignmentProposal proposal{assignment, {}};
    if (order.empty() || start + block_size > order.size()) {
        return proposal;
    }
    ...
    if (follow_previous) {
        ...
        for (std::size_t i = 0; i < block_size; ++i) {
            ...
            const depot_id_t from = proposal.assignment[customer_idx];
            if (from == depot) {
                continue;
            }
            proposal.assignment[customer_idx] = depot;
            proposal.changes.push_back({customer_idx, from, depot});
        }
        return proposal;
    }

    if (start + block_size >= order.size()) {
        return proposal;
    }
    const depot_id_t depot = assignment[order[start + block_size]];
    for (std::size_t i = 0; i < block_size; ++i) {
...
[[nodiscard]] AssignmentProposal make_block_set_depot_proposal(
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& order,
    const std::size_t start,
    const std::size_t block_size,
...
[[nodiscard]] AssignmentProposal make_swap_proposal(
    const std::vector<depot_id_t>& assignment,
    const std::size_t a,
    const std::size_t b
) {
    AssignmentProposal proposal{assignment, {}};
    if (a >= assignment.size() || b >= assignment.size() || a == b) {
        return proposal;
    }

    const depot_id_t da = assignment[a];
    const depot_id_t db = assignment[b];
    if (da == db) {
        return proposal;
    }

    proposal.assignment[a] = db;
    proposal.assignment[b] = da;
    proposal.changes.push_back({a, da, db});
    proposal.changes.push_back({b, db, da});
    return proposal;
}

[[nodiscard]] AssignmentProposal propose_guided_relocate(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& loads,
    const std::vector<cost_t>& ideal,
    const std::vector<std::vector<depot_id_t>>& nearest_candidates,
    const cost_t load_scale,
    AnnealRng& arng
) {
    AssignmentProposal proposal{assignment, {}};
    if (assignment.empty() || instance.depot_count() <= 1) {
        return proposal;
    }

    const depot_id_t source = select_source_depot_for_guided_move(loads, ideal, arng);
    const auto maybe_customer = pick_customer_assigned_to_depot(assignment, source, arng);
    if (!maybe_customer.has_value()) {
        return proposal;
    }

    const std::size_t customer_idx = *maybe_customer;
    const node_id_t customer_node = instance.customer_node_from_index(customer_idx);

    depot_id_t best_target = source;
    cost_t best_score = std::numeric_limits<cost_t>::infinity();

    for (const depot_id_t target : nearest_candidates[customer_idx]) {
        if (target == source) {
            continue;
        }

        const cost_t overload_after_target = std::max<cost_t>(
            0.0,
            static_cast<cost_t>(loads[target] + 1U) - ideal[target]
        );
        const cost_t overload_before_source = std::max<cost_t>(
            0.0,
            static_cast<cost_t>(loads[source]) - ideal[source]
        );
        const cost_t overload_after_source = std::max<cost_t>(
            0.0,
            static_cast<cost_t>(loads[source] > 0 ? loads[source] - 1U : 0U) - ideal[source]
        );
        const cost_t release_gain = overload_before_source - overload_after_source;

        cost_t score = matrix[target][customer_node];
        score += static_cast<cost_t>(0.60) * load_scale * overload_after_target;
        score -= static_cast<cost_t>(0.40) * load_scale * release_gain;
    ...
    proposal.assignment[customer_idx] = best_target;
    proposal.changes.push_back({customer_idx, source, best_target});
    return proposal;
}

[[nodiscard]] AssignmentProposal propose_distance_improving_swap(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::vector<depot_id_t>>& nearest_candidates,
    ...
    AssignmentProposal best{assignment, {}};
    if (assignment.size() < 2 || instance.depot_count() <= 1) {
        return best;
    }

    const std::size_t a = arng.next_index(assignment.size());
    const depot_id_t depot_a = assignment[a];
    const node_id_t node_a = instance.customer_node_from_index(a);
    cost_t best_delta = 0.0;

    const std::size_t trials = std::min<std::size_t>(assignment.size(), 8U);
    for (std::size_t t = 0; t < trials; ++t) {
        const std::size_t b = arng.next_index(assignment.size());
        if (a == b || assignment[b] == depot_a) {
            continue;
        }

        const depot_id_t depot_b = assignment[b];
        const node_id_t node_b = instance.customer_node_from_index(b);

        const bool depot_b_good_for_a =
            std::find(nearest_candidates[a].begin(), nearest_candidates[a].end(), depot_b) != nearest_candidates[a].end();
        const bool depot_a_good_for_b =
            std::find(nearest_candidates[b].begin(), nearest_candidates[b].end(), depot_a) != nearest_candidates[b].end();
        if (!depot_b_good_for_a && !depot_a_good_for_b) {
            continue;
        }

        const cost_t delta =
            matrix[depot_b][node_a] + matrix[depot_a][node_b] -
            matrix[depot_a][node_a] - matrix[depot_b][node_b];

        if (delta < best_delta - kEps) {
            best_delta = delta;
            best = make_swap_proposal(assignment, a, b);
        }
    }
...
[[nodiscard]] std::optional<MDMTSPSolution> evaluate_assignment_light(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    const std::vector<depot_id_t>& customer_to_depot,
...
[[nodiscard]] cost_t calibrate_initial_temperature(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    ...
    const auto orders = sa_customer_orders(instance, matrix);
    const cost_t start_proxy = assignment_access_proxy(instance, matrix, start_assignment, salesmen_per_depot);
    ...
    for (std::size_t s = 0; s < samples; ++s) {
        ...
        const std::size_t max_block = std::min<std::size_t>(sa_block_limit(instance.customer_count()), order.size());
        ...
        const auto depots = nearest_two_depots_for_customer(instance, matrix, order[start]);
        const auto proposal = make_block_set_depot_proposal(start_assignment, order, start, block_size, depots[1]);
        if (proposal.changes.empty()) {
            continue;
        }
        const cost_t delta =
            assignment_access_proxy(instance, matrix, proposal.assignment, salesmen_per_depot) - start_proxy;
        if (delta > kEps) {
...
[[nodiscard]] AssignmentProposal propose_assignment_sa_move_v4(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& loads,
    ...
    const std::vector<std::vector<depot_id_t>>& nearest_candidates,
    const cost_t load_scale,
    const cost_t temperature,
    const cost_t temperature0,
    AnnealRng& arng
) {
    AssignmentProposal proposal{assignment, {}};
    if (assignment.empty()) {
    ...
    const auto& order = orders[arng.next_index(orders.size())];
    const cost_t ratio = temperature0 <= kEps ? static_cast<cost_t>(0.0) :
        std::clamp(temperature / temperature0, static_cast<cost_t>(0.0), static_cast<cost_t>(1.0));

    std::size_t max_block = std::min<std::size_t>(sa_block_limit(instance.customer_count()), order.size());
    if (ratio < static_cast<cost_t>(0.20)) {
        max_block = std::min<std::size_t>(max_block, 2U);
    } else if (ratio < static_cast<cost_t>(0.45)) {
        max_block = std::min<std::size_t>(max_block, 4U);
    }
    const std::size_t block_size = 1U + arng.next_index(std::max<std::size_t>(1U, max_block));
    const std::size_t start_limit = order.size() >= block_size ? order.size() - block_size + 1U : 1U;
    const std::size_t start = arng.next_index(start_limit);

    const std::size_t roll = arng.next_index(100);
    if (ratio >= static_cast<cost_t>(0.55)) {
        if (roll < 18U) {
            return make_block_follow_proposal(assignment, order, start, block_size, true);
        }
        if (roll < 36U) {
            return make_block_follow_proposal(assignment, order, start, block_size, false);
        }
        if (roll < 60U) {
            const auto& depots = nearest_candidates[order[start]];
            return make_block_set_depot_proposal(assignment, order, start, block_size, depots.front());
        }
        if (roll < 78U) {
            const auto& depots = nearest_candidates[order[start]];
            const std::size_t target_pos = depots.size() <= 1U ? 0U : 1U + arng.next_index(depots.size() - 1U);
            return make_block_set_depot_proposal(assignment, order, start, block_size, depots[target_pos]);
        }
        if (roll < 92U) {
            return propose_guided_relocate(
                instance, matrix, assignment, loads, ideal, nearest_candidates, load_scale, arng
            );
        }
    } else {
        if (roll < 12U) {
    ...
    auto swap = propose_distance_improving_swap(instance, matrix, assignment, nearest_candidates, arng);
    if (!swap.changes.empty()) {
        return swap;
    }

    const std::size_t a = order[start];
    const std::size_t b = order[arng.next_index(order.size())];
    return make_swap_proposal(assignment, a, b);
}

[[nodiscard]] SaAssignmentResult solve_for_initial_assignment_by_sa_v4(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    std::vector<depot_id_t> start_assignment,
    ...
    auto current_solution = evaluate_assignment_light(
        instance, matrix, salesmen_per_depot, start_assignment, rng
    );
    if (!current_solution.has_value()) {
        throw std::logic_error("solve_for_initial_assignment_by_sa_v4: failed to build initial solution");
    }

    const auto depot_customer_targets =
        derive_customer_caps_from_salesmen(instance.customer_count(), salesmen_per_depot);
    const auto orders = sa_customer_orders(instance, matrix);
    const auto nearest_candidates = build_sa_nearest_depot_candidates(
        instance, matrix, sa_nearest_depot_candidate_count(instance)
    );
    const auto ideal = ideal_depot_loads(instance, salesmen_per_depot);
    const cost_t load_scale = std::max<cost_t>(1.0, mean_nearest_depot_distance(instance, matrix));

    auto current_assignment = std::move(start_assignment);
    auto current_loads = assignment_depot_loads(instance, current_assignment);
    cost_t current_proxy = assignment_access_proxy(instance, matrix, current_assignment, salesmen_per_depot);

    SaAssignmentResult best{
        current_assignment,
        current_loads,
        *current_solution,
        current_proxy
    ...
    AnnealRng arng{make_anneal_seed(instance, current_assignment)};
    cost_t temperature = calibrate_initial_temperature(
        instance, matrix, salesmen_per_depot, current_assignment, arng
    );
    const cost_t temperature0 = temperature;
    const double alpha = instance.customer_count() <= 500 ? 0.9945 : 0.9970;

    const auto started = std::chrono::steady_clock::now();
    const auto deadline = started + std::chrono::milliseconds(sa_time_budget_ms(instance.customer_count()));
    const std::size_t max_attempts = sa_max_attempts(instance.customer_count());
    const std::size_t materialize_stride = sa_materialize_stride(instance.customer_count());
    const std::size_t repair_stagnation_limit = sa_repair_stagnation_limit(instance.customer_count());
    const std::size_t frozen_limit = repair_stagnation_limit * 2U;

    std::size_t accepted_since_materialize = 0;
    ...
    while (attempts < max_attempts && std::chrono::steady_clock::now() < deadline) {
        ...
        const auto proposal = propose_assignment_sa_move_v4(
            instance,
            matrix,
            current_assignment,
            current_loads,
        ...
        if (proposal.changes.empty()) {
            temperature = std::max<cost_t>(temperature * static_cast<cost_t>(alpha), static_cast<cost_t>(1e-6));
            continue;
        }

        const cost_t proposal_proxy = assignment_access_proxy(
            instance, matrix, proposal.assignment, salesmen_per_depot
        );
        const cost_t delta = proposal_proxy - current_proxy;
        ...
        if (delta <= kEps) {
            ...
            const double p = std::exp(
                -static_cast<double>(delta / std::max<cost_t>(temperature, static_cast<cost_t>(1e-9)))
            );
            accept = arng.next_unit() < p;
        }

        if (accept) {
            current_assignment = proposal.assignment;
            apply_assignment_changes_to_loads(current_loads, proposal.changes);
            current_proxy = proposal_proxy;
            ++accepted_since_materialize;

            if (current_proxy < best.proxy - kEps) {
                best.assignment = current_assignment;
                best.loads = current_loads;
                best.proxy = current_proxy;
                accepted_since_materialize = materialize_stride;
            }
        }

        if (accepted_since_materialize >= materialize_stride) {
            auto materialized = evaluate_assignment_light(
                instance, matrix, salesmen_per_depot, current_assignment, rng
            );
            if (materialized.has_value() && solution_better_than(*materialized, best.solution)) {
                best.assignment = current_assignment;
                best.loads = current_loads;
                best.solution = *materialized;
                best.proxy = std::min(best.proxy, current_proxy);
                stagnation = 0;
                frozen = 0;
            } else {
                ++stagnation;
            ...
            accepted_since_materialize = 0;
        }

        if (instance.customer_count() <= kSaV4RepairCustomerThreshold &&
            stagnation >= repair_stagnation_limit &&
            std::chrono::steady_clock::now() < deadline) {
            auto repaired_assignment = best.assignment;
            refine_depot_assignment_by_soft_relocation(
                instance,
                matrix,
                depot_customer_targets,
            ...
            if (!assignment_equal(repaired_assignment, best.assignment)) {
                const cost_t repaired_proxy = assignment_access_proxy(
                    instance, matrix, repaired_assignment, salesmen_per_depot
                );
                auto repaired_solution = evaluate_assignment_light(
                    instance, matrix, salesmen_per_depot, repaired_assignment, rng
                );
                if (repaired_solution.has_value() &&
                    (solution_better_than(*repaired_solution, best.solution) || repaired_proxy < best.proxy - kEps)) {
                    best.assignment = std::move(repaired_assignment);
                    best.loads = assignment_depot_loads(instance, best.assignment);
                    best.solution = *repaired_solution;
                    best.proxy = repaired_proxy;
                }
            }

            current_assignment = best.assignment;
            current_loads = best.loads;
            current_proxy = best.proxy;
            temperature = std::max(temperature, temperature0 * (reheats == 0 ? static_cast<cost_t>(0.35) : static_cast<cost_t>(0.20)));
            stagnation = 0;
            frozen = 0;
            ++reheats;
        }

        if (temperature <= temperature0 * static_cast<cost_t>(0.03) && frozen >= frozen_limit) {
            break;
        }
    ...
    auto final_assignment = best.assignment;
    refine_depot_assignment_by_soft_relocation(
        instance,
        matrix,
        depot_customer_targets,
    ...
    if (!assignment_equal(final_assignment, best.assignment)) {
        const cost_t final_proxy = assignment_access_proxy(
            instance, matrix, final_assignment, salesmen_per_depot
        );
        if (auto final_solution = evaluate_assignment_light(
                instance, matrix, salesmen_per_depot, final_assignment, rng
            ); final_solution.has_value() &&
            (solution_better_than(*final_solution, best.solution) || final_proxy < best.proxy - kEps)) {
            best.assignment = std::move(final_assignment);
            best.loads = assignment_depot_loads(instance, best.assignment);
            best.solution = *final_solution;
            best.proxy = final_proxy;
        }
    }
...
[[nodiscard]] MDMTSPSolution finalize_sa_assignment_v4(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    const SaAssignmentResult& result,
    Random& rng
) {
    auto refined_assignment = result.assignment;
    bool refined = false;

    if (instance.customer_count() <= 1000) {
        const auto depot_customer_targets =
            derive_customer_caps_from_salesmen(instance.customer_count(), salesmen_per_depot);
        refine_depot_assignment_by_soft_relocation(
            instance,
        ...
        refined = !assignment_equal(refined_assignment, result.assignment);
    }

    if (instance.customer_count() <= kAssignmentRefinementCustomerThreshold) {
        if (auto improved = improve_assignment_by_gvns(
                instance,
                matrix,
                salesmen_per_depot,
                refined_assignment,
        ...
        if (auto light = evaluate_assignment_light(
                instance,
                matrix,
                salesmen_per_depot,
                refined_assignment,
                rng
            ); light.has_value() && solution_better_than(*light, result.solution)) {
            return *light;
        }
    ...
    if (instance.customer_count() <= kSaV4PortfolioFinalizeThreshold && refined) {
        if (auto portfolio = evaluate_assignment_portfolio(
                instance,
                matrix,
                salesmen_per_depot,
                refined_assignment,
            ...
            if (solution_better_than(*portfolio, result.solution)) {
                return *portfolio;
            }
        }
    }
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_simulated_annealing_v4(
    ...
    if (instance.customer_count() <= kSaV4SmallDelegationThreshold) {
        return solve_mdmtsp_simulated_annealing_v1(instance, rng);
    }

    std::optional<MDMTSPSolution> best_solution;
    std::optional<std::vector<depot_id_t>> baseline_assignment;
    if (instance.customer_count() <= kSaV4MediumBaselineThreshold) {
        auto baseline = solve_mdmtsp_simulated_annealing_v1(instance, rng);
        best_solution = baseline;
        baseline_assignment = assignment_from_solution(instance, baseline);
    }
    ...
    if (baseline_assignment.has_value()) {
        candidates.push_back(*baseline_assignment);
    }
    candidates.push_back(assign_customers_to_nearest_depots(instance, matrix));
    candidates.push_back(assign_customers_to_depots_balanced(instance, matrix, depot_customer_caps, rng));
    ...
    for (auto assignment : unique_assignments) {
        const auto annealed = solve_for_initial_assignment_by_sa_v4(
            instance,
            matrix,
            salesmen_per_depot,
            std::move(assignment),
        ...
        auto candidate = finalize_sa_assignment_v4(
            instance,
            matrix,
            salesmen_per_depot,
            annealed,
        ...
        if (!best_solution.has_value() || solution_better_than(candidate, *best_solution)) {
            best_solution = std::move(candidate);
        }
    }

    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_simulated_annealing_v4: failed to build any candidate");
    }
