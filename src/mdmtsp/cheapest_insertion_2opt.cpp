...
constexpr int kMaxPassesPerRoute = 6;
constexpr std::size_t kMaxChecksPerRoute = 750'000;
constexpr std::size_t kMinWindow = 8;
constexpr std::size_t kMaxWindow = 48;

[[nodiscard]] std::size_t two_opt_window_for_route(const Route& route) {
    if (route.size() <= 16) {
        return route.size();
    }
    const std::size_t scaled = route.size() / 32;
...
void improve_route_with_bounded_two_opt(Route& route,
                                        const DistanceMatrix& matrix,
                                        const bool return_to_depot) {
    if (route.size() < 4) {
        return;
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_cheapest_insertion_2opt(
    ...
    for (auto& route : solution.routes) {
        improve_route_with_bounded_two_opt(route.nodes, matrix, instance.return_to_depot);
    }
