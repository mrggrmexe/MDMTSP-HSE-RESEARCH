...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_lkh_mdmtsp_v4_portfolio(
    ...
    if (assignment_candidates.size() >= 2 && assignment_candidates[0] == assignment_candidates[1]) {
        assignment_candidates.pop_back();
    }

    const std::array<RouteSplitPolicy, 4> split_policies{
    ...
    for (const auto& assignment : assignment_candidates) {
        for (const auto split_policy : split_policies) {
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_lkh_mdmtsp_v4_portfolio: failed to build any candidate");
    }
