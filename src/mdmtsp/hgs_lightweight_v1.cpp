...
[[nodiscard]] std::vector<depot_id_t> hgs_assignment_crossover(
    ...
    const std::size_t max_block = std::max<std::size_t>(1, std::min<std::size_t>(order.size(), hgs_sa_block_limit(instance.customer_count()) * 3));
    ...
    while (selected.size() < population_limit) {
        ...
        for (std::size_t i = 0; i < population.size(); ++i) {
            ...
            for (const auto idx : selected) {
                min_dist = std::min(min_dist, assignment_hamming_distance(population[i].assignment, population[idx].assignment));
            }
            const cost_t rank = quality - static_cast<cost_t>(0.02) * static_cast<cost_t>(min_dist);
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_hgs_lightweight_v1(
    ...
    if (population.empty()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v1: empty initial population");
    }
    ...
    while (child_count < child_limit && std::chrono::steady_clock::now() < deadline) {
        ...
        std::size_t ib = tournament_pick(population, arng);
        ...
        const cost_t incumbent_proxy = best_solution.has_value()
        ...
        const bool should_materialize =
            ...
            (child.proxy < incumbent_proxy * static_cast<cost_t>(1.10));
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_hgs_lightweight_v1: failed to build any candidate");
    }
