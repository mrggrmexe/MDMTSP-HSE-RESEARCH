...
constexpr std::size_t kSaV3SmallDelegationThreshold = 120;
constexpr std::size_t kSaV3PortfolioFinalizeThreshold = 600;
constexpr std::size_t kSaV3RepairCustomerThreshold = 750;
...
[[nodiscard]] AssignmentProposal propose_assignment_sa_move_v2(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& loads,
...
[[nodiscard]] SaAssignmentResult solve_for_initial_assignment_by_sa_v2(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    std::vector<depot_id_t> start_assignment,
    ...
    if (!current_solution.has_value()) {
        throw std::logic_error("solve_for_initial_assignment_by_sa_v2: failed to build initial solution");
    }
    ...
    const double alpha = instance.customer_count() <= 500 ? 0.995 : 0.9975;
    ...
    while (attempts < max_attempts && std::chrono::steady_clock::now() < deadline) {
        ...
        const auto proposal = propose_assignment_sa_move_v2(
            instance,
            matrix,
            current_assignment,
            current_loads,
        ...
        if (instance.customer_count() <= kSaV3RepairCustomerThreshold &&
...
[[nodiscard]] MDMTSPSolution finalize_sa_assignment_v3(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    ...
    if (instance.customer_count() <= kSaV3PortfolioFinalizeThreshold && refined) {
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_simulated_annealing_v3(
    ...
    if (instance.customer_count() <= kSaV3SmallDelegationThreshold) {
    ...
    for (auto assignment : unique_assignments) {
        const auto annealed = solve_for_initial_assignment_by_sa_v2(
            instance,
            matrix,
            salesmen_per_depot,
            std::move(assignment),
        ...
        auto candidate = finalize_sa_assignment_v3(
            instance,
            matrix,
            salesmen_per_depot,
            annealed,
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_simulated_annealing_v3: failed to build any candidate");
    }
