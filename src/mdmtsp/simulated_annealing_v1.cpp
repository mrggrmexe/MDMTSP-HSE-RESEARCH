...
[[nodiscard]] std::vector<depot_id_t> assignment_set_block_to_depot(
    const std::vector<depot_id_t>& assignment,
    const std::vector<std::size_t>& order,
    const std::size_t start,
    const std::size_t block_size,
...
[[nodiscard]] std::vector<depot_id_t> assignment_swap_two(
    const std::vector<depot_id_t>& assignment,
    const std::size_t a,
    const std::size_t b
) {
...
[[nodiscard]] std::vector<depot_id_t> propose_assignment_sa_move(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<depot_id_t>& assignment,
    ...
    switch (op) {
        ...
        case 2: {
            ...
            return assignment_set_block_to_depot(assignment, order, start, block_size, depots[0]);
        }
        case 3: {
            const auto depots = nearest_two_depots_for_customer(instance, matrix, order[start]);
            return assignment_set_block_to_depot(assignment, order, start, block_size, depots[1]);
        }
        default: {
            const std::size_t a = order[start];
            const std::size_t b = order[arng.next_index(order.size())];
            return assignment_swap_two(assignment, a, b);
        }
    }
}
...
[[nodiscard]] cost_t calibrate_initial_temperature(
    ...
    for (std::size_t s = 0; s < samples; ++s) {
        const auto proposal = propose_assignment_sa_move(instance, matrix, start_assignment, orders, arng);
        if (proposal == start_assignment) {
            continue;
        }
        const cost_t delta =
            assignment_access_proxy(instance, matrix, proposal, salesmen_per_depot) - start_proxy;
        if (delta > kEps) {
...
[[nodiscard]] MDMTSPSolution solve_for_initial_assignment_by_sa(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
    std::vector<depot_id_t> start_assignment,
    ...
    if (!current_solution.has_value()) {
        throw std::logic_error("solve_for_initial_assignment_by_sa: failed to build initial solution");
    }
    ...
    std::vector<depot_id_t> best_assignment = current_assignment;
    MDMTSPSolution best_solution = *current_solution;
    cost_t best_proxy = current_proxy;
    ...
    while (attempts < max_attempts && std::chrono::steady_clock::now() < deadline) {
        ...
        const auto proposal_assignment = propose_assignment_sa_move(
            instance, matrix, current_assignment, orders, arng
        );
        if (proposal_assignment == current_assignment) {
        ...
        const cost_t proposal_proxy = assignment_access_proxy(
            instance, matrix, proposal_assignment, salesmen_per_depot
        );
        ...
        if (delta <= kEps) {
            ...
            const double p = std::exp(-static_cast<double>(delta / std::max<cost_t>(temperature, static_cast<cost_t>(1e-9))));
        ...
        if (accept) {
            ...
            if (current_proxy < best_proxy - kEps) {
        ...
        if (accepted_since_materialize >= materialize_stride) {
            ...
            if (materialized.has_value() && solution_better_than(*materialized, best_solution)) {
                best_solution = *materialized;
                best_assignment = current_assignment;
                best_proxy = current_proxy;
                stagnation = 0;
            } else {
        ...
        if (stagnation > 32 && std::chrono::steady_clock::now() < deadline) {
            current_assignment = best_assignment;
            current_proxy = best_proxy;
            temperature = std::max(temperature, temperature0 * static_cast<cost_t>(0.25));
            stagnation = 0;
        }
    ...
    if (auto final_solution = evaluate_assignment_light(
            instance, matrix, salesmen_per_depot, best_assignment, rng
        ); final_solution.has_value() && solution_better_than(*final_solution, best_solution)) {
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_simulated_annealing_v1(
    ...
    for (auto assignment : unique_assignments) {
        auto candidate = solve_for_initial_assignment_by_sa(
            instance,
            matrix,
            salesmen_per_depot,
            std::move(assignment),
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_simulated_annealing_v1: failed to build any candidate");
    }
