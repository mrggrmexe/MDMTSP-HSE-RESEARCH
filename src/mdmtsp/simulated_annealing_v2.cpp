...
[[nodiscard]] SaAssignmentResult solve_for_initial_assignment_by_sa_v2(
    ...
    while (attempts < max_attempts && std::chrono::steady_clock::now() < deadline) {
        ...
        if (stagnation >= repair_stagnation_limit && std::chrono::steady_clock::now() < deadline) {
...
[[nodiscard]] MDMTSPSolution finalize_sa_assignment_v2(
    const MDMTSPInstance& instance,
    const DistanceMatrix& matrix,
    const std::vector<std::size_t>& salesmen_per_depot,
...
[[nodiscard]] MDMTSPSolution solve_mdmtsp_simulated_annealing_v2(
    ...
    for (auto assignment : unique_assignments) {
        ...
        auto candidate = finalize_sa_assignment_v2(
            instance,
            matrix,
            salesmen_per_depot,
            annealed,
    ...
    if (!best_solution.has_value()) {
        throw std::logic_error("solve_mdmtsp_simulated_annealing_v2: failed to build any candidate");
    }
